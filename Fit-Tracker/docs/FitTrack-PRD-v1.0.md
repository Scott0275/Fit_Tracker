...
Retrieves list of users due for sync (last_sync + 15 min < now)
For each user, calls appropriate fitness API with stored OAuth tokens
Normalizes activity data to internal format
Deduplicates activities (same type + overlapping time window)
Calculates and awards points based on activity type and intensity
Updates user point balance and activity log
...
User-designated primary tracker
Most detailed data (more metrics available)
First received
4.3 Points System
...
T-5 minutes: Ticket sales close for the drawing
T-0: Drawing execution begins
...
T+1 minute: Winners notified via email and in-app push
T+5 minutes: Results published to all users
Ongoing: Prize fulfillment workflow initiated
...
Earlier achievement of the point total
More active days in period
User ID (deterministic fallback)
...
5.4 Security
See Section 10: Security Considerations for detailed requirements.
...
Random Number Generation: OCI Vault CSPRNG
Audit Trail: Immutable log of all drawing actions
Admin Authentication: MFA required for drawing execution
Ticket Snapshot: Immutable snapshot created at sales close
Result Publication: Automated, no manual editing possible
Third-Party Audit: Annual audit by independent firm
...
New user registration → profile completion → tracker connection → first activity sync
Point earning → ticket purchase → drawing entry → result viewing
Admin drawing creation → scheduling → execution → winner notification
Premium upgrade flow (v1.1)
...
Detect failure (automated health checks)
Assess scope (single service vs. regional outage)
Execute runbook (automated where possible)
Validate recovery
